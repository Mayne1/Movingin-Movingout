	...
	implements KTColorPickerInterface
{
	...
	protected _buildPickrOptions(): any {
		...
		// Theme
		const theme = this._getOption('theme') as string;
	...
	protected _setupPickrEvents(): void {
		...
		this._pickr.on('init', () => {
			...
			if (inputMode && this._element && this._element.tagName === 'INPUT') {
				...
				if (ktInputContainer && !ktInputContainer.querySelector('.kt-color-picker-preview')) {
					...
					// Add padding to input to make room for preview box (0.25rem left + 1.75rem box + 0.5rem gap = 2.5rem)
		...
		// Show event - reposition if input mode and attach focus handler
		if (inputMode) {
			...
			this._pickr.on('show', () => {
				self._repositionToParentContainer();
			});
		} else {
			// For non-input mode, ensure Pickr positions correctly
			// Add a small delay to let Pickr calculate position first
			this._pickr.on('show', () => {
				setTimeout(() => {
					const root = self._pickr!.getRoot();
					const rootElement = ((root as any).app || root) as HTMLElement;
					if (!rootElement || !self._element) return;

					// Check if position is at top: 0px (default/incorrect position)
					const currentTop = parseFloat(rootElement.style.top) || 0;
					if (currentTop === 0 && self._element) {
						// Get element position and adjust
						const elementRect = self._element.getBoundingClientRect();
						const pickerRect = rootElement.getBoundingClientRect();

						// Position below the element with a small gap
						const gap = 8;
						rootElement.style.top = `${elementRect.bottom + gap + window.scrollY}px`;

						// Align left edge with element
						rootElement.style.left = `${elementRect.left + window.scrollX}px`;
					}
				}, 10);
			});
		}
		...
		// Clean up click handler on hide
		this._pickr.on('hide', () => {
			const root = self._pickr!.getRoot();
			const rootElement = ((root as any).app || root) as HTMLElement;
	...
	protected _repositionToParentContainer(): void {
		if (!this._pickr || !this._element) return;
		...
		const root = this._pickr.getRoot();
		const rootElement = ((root as any).app || root) as HTMLElement;
		if (!rootElement) return;
		...
		// Calculate the vertical offset - position below the container
		const topOffset = containerRect.bottom - inputRect.bottom;
		const currentTop = parseFloat(rootElement.style.top) || 0;
		rootElement.style.top = `${currentTop + topOffset}px`;
		...
		switch (positionToInput) {
			case 'left':
				// Align picker's left edge with container's left edge
				newLeft = containerRect.left;
				break;
			case 'right':
				// Align picker's right edge with container's right edge
				newLeft = containerRect.right - pickerRect.width;
				break;
			case 'center':
				// Center picker relative to container
				newLeft = containerRect.left + (containerRect.width - pickerRect.width) / 2;
				break;
			case 'auto':
			default:
				// For auto, keep the original position but adjust for container
				const leftOffset = inputRect.left - containerRect.left;
				const currentLeft = parseFloat(rootElement.style.left) || 0;
